...
这个页面要渲染的是 259 条静态数据，客户端状态只有"一个输入框"和"悬停展开"。
...
| `src/data/categories.ts` | **站点数据**：20 个分类、259 个条目 |
| `src/data/engines.ts` | 搜索框上方的 9 个外部搜索引擎 |
| `src/data/types.ts` | 数据模型 |
| `docs/aa-import.md` | aa 站点补录的来源、去重规则与跳过清单 |
...
```
  · 20 个分类 / 259 个站点（有图标 75，首字母兜底 184）
  · 图标库：header 15 · icon 69 · search 9
  · 搜索引擎 9 个，默认「bing」

  ! 有 14 个图标没被任何条目引用（不进产物，仅占仓库体积）：icon/bing.png、…
```

闲置图标只是仓库卫生问题，**不影响线上**：Vite 只打包被引用到的资源，
这些文件根本不会进 `dist/`（实测磁盘上 96 张 PNG，产物里 77 张）。
...
## 内容来源：aa（AnywhereAnything）补录

站点数据在原有 97 条的基础上，从 [lackar.com/aa](http://lackar.com/aa/) 补录了 162 条，
现为 **20 个分类 / 259 个条目**。原站已整站下线（`lackar.com` 现在返回 DreamHost 的
`Site not found`），数据取自 **Common Crawl `CC-MAIN-2019-18` 的 2019-04-22 快照**。
取证方式、去重规则、分类映射、跳过清单都在 [`docs/aa-import.md`](docs/aa-import.md)。

三件事值得单独说：

- **原有 97 条一条没动** —— 分类、顺序、名称、URL、图标键全部原样保留；新增条目
  统一追加在各分类 `sites` 末尾，并带 `// ---- 以下为 aa 补录 ----` 分隔注释。
- **新增了 `电影` / `应用` / `搜索` 三个分类**（aa 有、本项目没有）。它们排在
  锚点导航最后，**原有 17 个分类的相对顺序不变**。三个推荐位图标分别来自
  豆瓣（复用已有的 100×100 资源）、App Store（`apps.apple.com` 的 180×180
  apple-touch-icon）、Google（复用已有资源）。
- **新增条目基本没有图标**：162 条里只有 CocoaChina 能对上图标池里的真实 Logo，
  其余一律渲染成首字母色块（补录后：有图标 75 条 / 兜底 184 条）。
  这是刻意的 —— 池子里剩下的公司 Logo 大多是别家的，套到新站点上比留白更糟。
  想补真实图标，把文件丢进 `icon/<任意目录>/` 再填上键名即可。

去重按**域名**做（忽略 `www.`，子域算不同站），所以 `wikihow.com` 这种首页/子页
不同但同站的不会重复收录。补录后文件里还剩 5 处重复 URL，**全部是原有数据自带的**
（知乎跨「知识/社交」、Pinterest 推荐位与「设计」、Bilibili 跨「视频/旅行」、
`kernel.org` 被三个分类当推荐位），本次没有触碰。
